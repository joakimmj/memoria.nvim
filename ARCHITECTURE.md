...
- [Part 3 — memoria.nvim](#part-3-memorianvim)
  ...
  - [5. Synapses (engram-to-engram connections)](#5-synapses-engram-to-engram-connections)
    ...
    - [5.5 Parse](#55-parse)
    - [5.6 add_synapse (inverse sync)](#56-add_synapse-inverse-sync)
  ...
  - [8. Creating and appending to engrams](#8-creating-and-appending-to-engrams)
    - [8.1 `add_engram(brain_name, opts?)`](#81-add_engrambrain_name-opts)
...
| `section.get` | `parse_synapse_block` (§5.5), `append_to_engram` (§8.2) |
| `section.set` | `write_synapse_block` (§5.4), `append_to_engram` (§8.2), the generated index (§9.3) |
...
- **A minimal-span buffer write.** Whenever memoria rewrites an engram that is
  ...
  the span between, for the extmark reason in Part 2 §2. `md-drafting`'s own
  `util.replace_lines` is internal and not to be required.
...
moment Neovim starts rather than the moment `add_engram` is first called.
...
| `commands.lua` | Every `:Mia*` command, created by `setup()` when `add_commands` is on | — |
| `config.lua` | Built-in defaults and the tier merge (§3) | — |
| `modules/` | One file per feature — `brain`, `engram`, and later `synapse`, `atlas`, … | a field on the plugin table |
| `lib/` | Shared machinery — `md-drafting` (the only path to the dependency, §1.1), `json`, `date`, `synapse` (block read/write, §5.4–5.5) | internal only |

Commands live in `lua/` rather than `plugin/` because they may only exist once
the guard has passed, which `setup()` decides.

`synapse` appears twice on purpose: `lib/synapse.lua` reads and writes the block
over lines, the way `md-drafting`'s `section` does, and `modules/synapse.lua`
holds `add_synapse` (§5.6), the feature built on it.

Lua functions are snake_case, matching `md-drafting`: `add_engram`,
`write_synapse_block`.
...
Created only when `add_commands = true` (§3), the same opt-in `md-drafting` uses.
Unlike `md-drafting`'s, they are global rather than buffer-local: a brain is
managed from anywhere, not only from inside a markdown buffer. Every command is
a thin wrapper over a Lua function, so nothing here needs the command to exist.

| Command | Behavior |
|---|---|
| `:MiaBrainAdd <path> [name]` | Creates `path` if missing. `name` defaults to `basename(path)`, overridable. Errors on name collision. |
| `:MiaBrainDeregister <name>` | Removes the registry entry **only** — never touches files on disk. |
...
| `:MiaBrainCurrent` | Names the brain a command would act on right now, and why — the buffer's brain, or the active one. |
| `:MiaBrainSwitch <name>` | Sets the active brain for the session. |
...
| `:MiaEngramAdd [brain]` | Creates an engram (`add_engram`, §8.1) in the given brain. |
...
3. the active brain
4. a picker over registered brains
```
...
That also means the active brain is not always the one acted on, which is what
`:MiaBrainCurrent` answers and `:MiaBrainList`'s two markers show.
...
require('memoria').setup({
  add_commands = false,          -- create the :Mia* commands (§2.2)
  engrams = {
...
first two. A JSON `null` is not a value: it leaves the default in place.
...
```
prompt_prefix_concept():
  1. show picker: existing keys in mia_concepts.json (fuzzy-searchable)
     + a "+ Create new concept" entry
  2a. existing concept selected → use its name as the prefix
  2b. "create new" selected →
        prompt for name (+ type)
        set_concept_meta(name, type, {})   // registers it, meta can stay empty
        use new name as the prefix
...
- **No blank line before the heading.** An ATX heading needs none after an HTML
  comment; markdownlint's MD022 disagrees, which is a linter preference, not a
  rendering issue.
...
write_synapse_block(lines, engram, synapse_fields):
  1. body = []
  2. for field in synapse_fields where target == "engram", sorted by name:
       values = engram.synapses[field.name] || []
       if values is empty and field.show_empty == false: continue
       link_str = values.map(v => drafting.api.syntax.format_link(v.title, v.path)).join(", ")
       text = `**${field.name}:**` + (link_str == "" ? "" : " " + link_str)
       body.push(drafting.api.syntax.format_list_item("- ", nil, text))
     body.push("***")
  3. _, fm_end = drafting.api.syntax.parse_frontmatter(lines)
...
### 5.6 add_synapse (inverse sync)

```
add_synapse(source_id, target_id, synapse_type):
  1. load source engram, load target engram
  2. append target_id to source synapses[synapse_type]
  3. append source_id to target synapses[inverse_of(synapse_type)]
  4. write each file with write_synapse_block(its_lines, source|target, ...)
  5. update atlas for both engrams
```
...
```
find_undeclared_concepts():
  1. mentions = extract all concept-mentions (participants fields, plain tags, etc.) across engrams
  2. group by text, count occurrences
  3. undeclared = [ m for m in mentions if resolve_concept(m) is null ]
  4. return undeclared, sorted by count desc
```

```
set_concept_meta(name, type, fields):
  1. entry = concepts[name] || { type }
  2. schema = concepts[type]?.fields || []
  3. entry.meta = entry.meta || {}
  4. for key in fields:
       entry.meta[key] = fields[key]   // unknown keys allowed, just warn
  5. concepts[name] = entry; write mia_concepts.json
```
...
- `concepts_by_type` exists purely to make `concept_type`-filtered pickers fast
  ...
  appears here, grouped under whatever string its `type` happens to be.
...
rebuild_atlas(brain):
  ...
  report:
    ...
    - missing inverse synapses (self-healable: call add_synapse's inverse-write)
    - undeclared concepts in use
    - orphaned concepts (registered, zero engrams reference them)
```

Also the natural place to backfill synapse-block lines for engrams predating a
newly-added synapse field in config (config always wins going forward; old
engrams aren't retroactively rewritten until this runs).

---

## 8. Creating and appending to engrams

### 8.1 `add_engram(brain_name, opts?)`
...
```
add_engram(brain_name, opts?):
  1. resolve brain config
  2. determine filename per config.engrams.filename (§4)
     - prompt "Engram title:", seeded with opts.title if given
  ...
  3. header_lines =
     ...
     generated_header = write_synapse_block(header_lines, empty_engram, config.synapses where target == "engram")
       ...
       // the *** separator is part of the block
  4. prose = render_template(config.engrams.content_template, { title, date })
  5. content = generated_header + prose
  6. cursor_pos = locate "%cursor%" in rendered prose (default to end-of-content if absent)
  7. write file (with %cursor% stripped), register in atlas
  8. open buffer, place cursor at cursor_pos
```
...
Default `content_template`: `"\n# %title%\n\n%cursor%"` — a blank line under
the generated header, the heading, a blank line, cursor ready to write.

`add_engram` always opens the buffer — you're starting something to actively
write.
...
Unlike `add_engram`, this never opens a buffer — the entire point is avoiding a
...
`resolve_concept` (§6.3), engram lookup, and `add_engram` (§8.1); uses
...
link_or_create_at_cursor():
  ...
  3b. not resolved →
        ...
        add_engram(current_brain, { title = text })
...
open_or_create_today(brain_name):
  ...
  4. else → add_engram(brain_name, { title = "diary" })
...
`add_synapse` keeps both sides in sync — deleting should undo that sync, not leave
...
- **Engram header is laid out for vertical space** (§5.1): flow-style
  ...
  breaks without hard-break syntax), a `***` separator inside the section, no
  blank line before the heading. Kept at the top, next to the frontmatter.
...
- **`add_engram` opens the buffer; `append_to_engram` never does** — deliberately
  asymmetric, matching their different purposes (start writing vs. quick capture
  without interrupting current work).
