...
--- Echo the current brain, or say there is none.
function M.print_current()
  local brain, reason = M.current()
  if not brain then
    vim.notify("memoria: no current brain; :MiaBrainSwitch or open an engram", vim.log.levels.WARN)
    return
  end

  vim.api.nvim_echo({
    { ("%s — %s\n"):format(brain.name, reason) },
    { "  " .. brain.location },
  }, false, {})
end
